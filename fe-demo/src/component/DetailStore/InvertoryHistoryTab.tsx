...
export default function InventoryHistoryTab(){
  ...
  const data = {
    ...
    content:[
      {
        id:1,
        ma_kiem_ke:'KKD3/2026',
        ten_dot_kiem_ke:'Kiểm kê tháng 3',
        nguoi_kiem_ke:'Kiểm kê tháng 3',
        loai_kiem_ke:'Đột xuất',
        thoi_gian_kiem_ke:'2026-04-10 14:30:00'
      }
    ],
    totalPages:1,
    totalElements:1,
  ...
  const column: Column<any>[]=[
    ...
    },
    {
      ...
      render:(item:any)=>(
        <span className="text-normal-14">{item.ten_dot_kiem_ke}</span>
      )
    },
    {
      key:'nguoi_kiem_ke',
      label:'Người kiểm kê',
      render:(item:any)=>(
        <span className="text-normal-14">{item.nguoi_kiem_ke}</span>
      )
    },
    {
      key:'loai_kiem_ke',
      label:"Loại kiểm kê",
      render:(item:any)=>(
        <span className={getClassCssLoaiKiemKe(item.loai_kiem_ke)}>{item.loai_kiem_ke}</span>
      )
    },
    {
      key:'thoi_gian_kiem_ke',
      label:'Thời gian kiểm kê',
      render:(item:any)=>(
        <span className="text-ibm-mono-14">{item.thoi_gian_kiem_ke}</span>
      )
    }
  ]
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      <div className="flex gap-[50px] items-center">
        <SearchBar search={search} handleSearchChange={setSearch} placeholder="Tìm kiếm theo mã kiểm kê hoặc tên đợt kiểm kê"/>
