...
export type FormFieldType = 'text' | 'date' | 'select' | 'textarea' | 'file';
            ...
            {(() => {
              ...
              while (i < section.fields.length) {
                ...
                if (field1 && isFullWidth(field1)) {
                   rows.push(
                     <div key={field1.name} className="flex w-full flex-col items-start gap-2">
                       <div className="flex">
                   ...
                   // Render 2 cols
                   rows.push(
                     <div key={`${field1.name}-${field2.name}`} className="grid grid-cols-2 gap-6 max-sm:grid-cols-1">
                       {[field1, field2].map((field) => (
                         <div key={field.name} className="flex flex-col gap-2">
                           ...
                           <div className={field.hasSearchBtn ? "flex gap-[10px]" : ""}>
                             {field.type === 'select' ? (
                               <select
                                 ...
                                 className={`text-normal h-8 w-full rounded border-[0.8px] border-[#D9D9D9] bg-white px-4 ${
                                   formData[field.name] ? "text-black/85" : "text-black/25"
                                 }`}
                               >
                   ...
                   rows.push(
                     <div key={field1.name} className="grid grid-cols-2 gap-6 max-sm:grid-cols-1">
                       <div className="flex flex-col gap-2">
                         <div className="flex">
                           <span className="text-normal mr-1 font-medium">{field1.label}</span>
                           {field1.required && <img src={required_icon} alt="required" />}
                         </div>
                         <div className={field1.hasSearchBtn ? "flex gap-[10px]" : ""}>
                           {field1.type === 'select' ? (
                             <select
                               value={formData[field1.name] || ""}
                               onChange={handleInputChange(field1.name)}
                               disabled={field1.disabled}
                               className={`text-normal h-8 w-full rounded border-[0.8px] border-[#D9D9D9] bg-white px-4 ${
                                 formData[field1.name] ? "text-black/85" : "text-black/25"
                               }`}
                             >
                               <option value="" disabled>
